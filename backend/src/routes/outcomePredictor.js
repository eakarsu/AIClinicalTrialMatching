/*
 * Outcome predictor: Forecast dropouts and adverse events early
 *
 * Custom Feature Suggestion (apply pass 6 — audit batch_01)
 * Suggestion: Forecast dropouts and adverse events early
 *
...
router.post('/run', async (req, res) => {
  try {
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({ error: 'AI unavailable', missing: 'OPENROUTER_API_KEY', feature: 'outcome_predictor' });
    }
    ...
    const systemPrompt = `You are a $'clinical trial outcome predictor'. Respond with strict JSON.`;
    const aiResp = await callOpenRouter(userPrompt, systemPrompt);
    const content = aiResp?.choices?.[0]?.message?.content ?? aiResp?.raw ?? '';
    const parsed = parseAIJson(content);
    return res.json({ feature: 'outcome_predictor', model: aiResp?.model, parsed, raw: content });
  } catch (err) {
...
router.get('/health', (req, res) => res.json({ feature: 'outcome_predictor', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
