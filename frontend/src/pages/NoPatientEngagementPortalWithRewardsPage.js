...
export default function NoPatientEngagementPortalWithRewardsPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-no-patient-engagement-portal-with-rewards/run', { input });
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No patient-engagement portal with rewards</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-patient-engagement-portal-with-rewards/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
