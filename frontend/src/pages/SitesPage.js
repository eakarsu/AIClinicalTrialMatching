...
export default function SitesPage() {
  ...
  useEffect(() => { loadData(); }, []);
  const loadData = async () => { try { const res = await api.get('/sites'); setItems(Array.isArray(res.data) ? res.data : (res.data?.items || res.data?.data || res.data?.rows || [])); } catch(e){} setLoading(false); };
