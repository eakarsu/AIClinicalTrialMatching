...
export default function NoRemoteMonitoringTelevisitModulePage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-no-remote-monitoring-televisit-module/run', { input });
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No remote-monitoring/televisit module</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-remote-monitoring-televisit-module/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
