...
// Feature: Patient outcome predictor ML model forecasting dropouts and adverse events early
import React, { useState } from 'react';
import api from '../services/api';

export default function PatientOutcomePredictorMlModelForecastingDroPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-patient-outcome-predictor-ml-model-forecasting-dro/run', { input });
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Patient outcome predictor ML model forecasting dropouts and adverse events early</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-patient-outcome-predictor-ml-model-forecasting-dro/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
