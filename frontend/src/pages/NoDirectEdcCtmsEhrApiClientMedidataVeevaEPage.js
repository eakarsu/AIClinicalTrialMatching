...
export default function NoDirectEdcCtmsEhrApiClientMedidataVeevaEPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-no-direct-edc-ctms-ehr-api-client-medidata-veeva-e/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No direct EDC/CTMS/EHR API client (Medidata, Veeva, Epic) beyond webhook scaffold</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-direct-edc-ctms-ehr-api-client-medidata-veeva-e/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
