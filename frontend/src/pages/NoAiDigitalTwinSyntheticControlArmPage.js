...
export default function NoAiDigitalTwinSyntheticControlArmPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-no-ai-digital-twin-synthetic-control-arm/run', { input });
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No AI digital twin / synthetic control arm</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-ai-digital-twin-synthetic-control-arm/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
