...
export default function NoEConsentModuleWithAuditTrailPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-no-e-consent-module-with-audit-trail/run', { input });
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No e-consent module with audit trail</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-e-consent-module-with-audit-trail/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
