...
// Feature: Regulatory compliance agent auto-generating amendments, adverse-event reports, and submissions
import React, { useState } from 'react';
import api from '../services/api';

export default function RegulatoryComplianceAgentAutoGeneratingAmendmPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-regulatory-compliance-agent-auto-generating-amendm/run', { input });
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Regulatory compliance agent auto-generating amendments, adverse-event reports, and submissions</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-regulatory-compliance-agent-auto-generating-amendm/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
